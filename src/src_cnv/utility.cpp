			...
			for (int k = 0; k < seqs[0]->size(); k++) {
				if (!(seqs[i]->at(k) == 45 && seqs[j]->at(k) == 45)) {
					if (seqs[i]->at(k) == 45 || seqs[j]->at(k) == 45)
						...
						val += subsmat[subsref[seqs[i]->at(k)]]->at(subsref[seqs[j]->at(k)]) - round((opn_pen + ext_pen * gap_len + cnv_pen * pen_fun(gap_len)) * 10000) / 10000;
						gap_len = 0;
					}
				}
			}
			if (gap_len > 0) {
				val -= round((opn_pen + ext_pen * gap_len + cnv_pen * pen_fun(gap_len)) * 10000) / 10000;
				gap_len = 0;
			}
		}
	}
...
void get_ext_ref(vector<vector<double> >& vec) {			//vector giving the extention cost of extending a gap of length "row number" by a gap of length "column number"
	...
	for (int i = 1; i < M + 2; ++i) 
		vec[0].emplace_back(-round((opn_pen + ext_pen * i + cnv_pen * pen_fun(i)) * 10000) / 10000);
	for (int j = 1; j < M + 2; ++j) {
		vector<double> temp_vec;
		temp_vec.reserve(M + 2);
		vec.emplace_back(temp_vec);
		...
		for (int i = 1; i < M + 2; ++i)
			vec[j].emplace_back(-round((ext_pen * i + cnv_pen * (pen_fun(i + j) - pen_fun(j))) * 10000) / 10000);
	}
}
...
void det_fun(int input) {
	...
	if (input == 1)
		...
		cout << "\n!!!!!!!!!!!!!!!!!!!!!!!!!! Penalty function choice is invalid, using default penalty function !!!!!!!!!!!!!!!!!!!!!!!!!!\n\n";
}
